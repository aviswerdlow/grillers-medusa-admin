...
export class Migration20260920150000 extends Migration {
  async up(): Promise<void> {
    this.addSql(`create table gp_incoming_batch (
    ...
    this.addSql(`create table gp_incoming_demand (
    ...
    this.addSql(`create table gp_incoming_commitment (
    ...
    this.addSql(`create table gp_incoming_receipt (
    ...
    this.addSql(`create table gp_incoming_event (
    ...
    this
      .addSql(`create index gp_incoming_batch_supply on gp_incoming_batch (variant_id,status,usable_at);
      create unique index gp_incoming_demand_cart_line on gp_incoming_demand (cart_id,line_item_id) where cart_id is not null and status = 'committed';
      create unique index gp_incoming_demand_order_line on gp_incoming_demand (order_id,line_item_id) where order_id is not null and status = 'committed';
      create index gp_incoming_commitment_batch on gp_incoming_commitment (batch_id);
      create index gp_incoming_demand_order on gp_incoming_demand (order_id);
      create index gp_incoming_event_variant on gp_incoming_event (variant_id,created_at);`);
  }
