...
import { Modules, toMikroOrmEntities, isSharedConnectionSymbol } from "@medusajs/framework/utils"
...
const schema = `gp_inventory_${randomUUID().replace(/-/g, "")}`
...
beforeAll(async () => {
  ...
  if (!url || !redisUrl || ![new URL(url), new URL(redisUrl)].every(u => ["localhost", "127.0.0.1"].includes(u.hostname))) {
    throw new Error("Supply isolated loopback INVENTORY_TEST_DATABASE_URL and INVENTORY_TEST_REDIS_URL; never production variables")
  }
  db = knex({ client: "pg", connection: url, searchPath: [schema] })
  // Medusa's DML index expressions are unqualified. Keep their creation in this
  // disposable schema as well as qualifying the ORM entity tables.
  const isolatedUrl = new URL(url)
  isolatedUrl.searchParams.set("options", `-c search_path=${schema}`)
  wrapper = getMikroOrmWrapper({ mikroOrmEntities: toMikroOrmEntities(Object.values(models)), clientUrl: isolatedUrl.toString(), schema })
  ...
  const result = await initModules({
    ...
    modulesConfig: { [Modules.INVENTORY]: { resolve: "@medusajs/inventory", options: {
      database: { clientUrl: url, schema }, [isSharedConnectionSymbol]: true,
    } } },
  ...
  locks = clients.map(redisClient => new RedisLockingProvider({ redisClient, prefix: `${schema}:` }, {}))
...
afterAll(async () => {
  ...
  if (db) { await db.raw(`drop schema if exists ${schema} cascade`); await db.destroy() }
})
beforeEach(async () => {
...
it("permits exactly one native last-unit reservation across independent Redis clients", async () => {
  ...
  const item = await inventory.retrieveInventoryItem(v.itemId, { relations: ["location_levels"] })
  expect(item.location_levels[0]).toMatchObject({ stocked_quantity: 1, reserved_quantity: 1, available_quantity: 0 })
})
